...
And let’s reboot again. I'm not sure if this is really necessary, but I had a couple of times
...
Let’s add some lines to  `/etc/hosts` that will make it easier for us to refer to each node from any
node (not sure if it’s required to be honest, but it's what I did):
